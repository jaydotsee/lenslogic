...
The wizard takes about 5-10 minutes to complete.
        ...
        return questionary.confirm(
            "Would you like to continue with the configuration wizard?",
            default=True
        ).ask()
    ...
    def _configure_naming_patterns(self):
        ...
        self.console.print("[dim]                    {date}, {time}, {camera}, {original_name}, {iso}, {f_number}[/dim]")
    ...
    def _configure_advanced_features(self):
        ...
        session_detection = questionary.confirm(
            "Enable shooting session detection?",
            default=True
        ).ask()
    ...
    def _configure_backup_settings(self):
        ...
        if enable_backup:
            ...
            verify_backups = questionary.confirm(
                "Enable backup verification (checksum validation)?",
                default=True
            ).ask()
    ...
    def _show_configuration_summary(self):
        ...
        table = Table(title="Your LensLogic Configuration", show_header=True, header_style="bold magenta")
        table.add_column("Setting", style="cyan", width=30)
        table.add_column("Value", style="green")

        # Basic settings
        table.add_row("Source Directory", self.config_manager.get('general.source_directory', 'Not set'))
        table.add_row("Destination Directory", self.config_manager.get('general.destination_directory', 'Not set'))
        table.add_row("Preserve Originals", str(self.config_manager.get('general.preserve_originals', True)))

        # Organization
        table.add_row("Folder Structure", self.config_manager.get('organization.folder_structure', 'Default'))
        table.add_row("Separate RAW/JPEG", str(self.config_manager.get('organization.separate_raw', True)))
        table.add_row("Naming Pattern", self.config_manager.get('naming.pattern', 'Default'))

        # Features
        table.add_row("Geolocation Enabled", str(self.config_manager.get('geolocation.enabled', True)))
        table.add_row("Duplicate Detection", str(self.config_manager.get('features.remove_duplicates', True)))
        table.add_row("Auto-rotate Images", str(self.config_manager.get('image_processing.auto_rotate', True)))

        # Backup
        backup_destinations = self.config_manager.get('backup.destinations', [])
        table.add_row("Backup Destinations", f"{len(backup_destinations)} configured" if backup_destinations else "None")
    ...
    def quick_setup(self) -> bool:
        ...
        try:
            ...
            self.config_manager.set('naming.pattern', '{year}{month:02d}{day:02d}_{hour:02d}{minute:02d}{second:02d}_{camera}_{original_name}')
