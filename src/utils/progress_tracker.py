...
class ProgressTracker:
    ...
    def update_file(self, file_path: str, status: str = "processing"):
        ...
        if self.verbose and self.progress:
            self.progress.update(
                self.task_id,
                description=f"[cyan]{status}: {Path(file_path).name}"
            )

    def file_processed(self, success: bool = True, action: str = None,
                      size: int = 0, skipped: bool = False):
    ...
    def print_summary(self):
        ...
        success_rate = 0
        if self.stats['processed_files'] > 0:
            success_rate = ((self.stats['processed_files'] - self.stats['failed_files'])
                          / self.stats['processed_files'] * 100)
    ...
    def display_file_preview(self, original: str, new_name: str,
...
"""
    ...
    def create_statistics_table(self, stats: Dict[str, Any]) -> Table:
        table = Table(title="Library Statistics", show_header=True,
