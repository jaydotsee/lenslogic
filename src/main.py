            ...
            try:
                ...
                if dry_run:
                    ...
                    result = self.folder_organizer.organize_file(
                        str(file_path), dest_folder, new_filename,
                        dry_run=False, preserve_original=preserve_originals
                    )
                    ...
                    if result['success']:
                        ...
                        if create_sidecar:
                            self.folder_organizer.create_sidecar_files(
                                str(file_path), str(dest_folder / new_filename),
    ...
    def detect_sessions(self, organize_by_sessions: bool = False):
        ...
        if organize_by_sessions:
            ...
            result = self.session_detector.organize_by_sessions(
                sessions, dest_dir, "{session_name}",
                dry_run=self.config.get('general', {}).get('dry_run', False)
            )

            self.progress_tracker.print_success(f"Organized {result['files_organized']} files into {result['sessions_processed']} session folders")

    def optimize_for_social_media(self, platform: str, format_type: str = 'post',
            ...
            for dest in destinations:
                ...
                if verification['integrity_score'] >= 95:
                    ...
                    if verification['corrupted_files']:
                        self.progress_tracker.print_warning(f"    Corrupted files: {len(verification['corrupted_files'])}")
    ...
    def show_exif_info(self):
        ...
        if files:
            ...
            try:
                ...
                fields_count = len([k for k, v in metadata.items() if v is not None and k not in ['file_path', 'file_name']])
...
def main():
    ...
    parser.add_argument('--analyze-xmp', action='store_true', help='Analyze library using XMP sidecar files')
    parser.add_argument('--xmp-library', type=str, help='Library directory for XMP analysis (default: source directory)')
    parser.add_argument('--xmp-output', type=str, help='Output directory for detailed XMP analysis reports')
    parser.add_argument('--advanced-stats', action='store_true', help='Generate comprehensive statistics with charts')
    ...
    parser.add_argument('--social-media', type=str, choices=['instagram', 'facebook', 'twitter', 'linkedin'],
                       help='Optimize images for social media platform')
    parser.add_argument('--social-format', type=str, default='post',
    ...
    # Legacy options
    parser.add_argument('--export-gps', type=str, metavar='FILE', help='Export GPS locations to KML file')
    ...
    if args.interactive:
        ...
        if not destinations:
            ...
            for dest in destinations:
                ...
                organizer.progress_tracker.print_info(f"Verification complete for {dest}: {verification['integrity_score']:.1f}% integrity")
